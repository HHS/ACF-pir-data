...
class MockData:
    ...
    def data_specifications(self) -> Self:
    ...
    def generate_rows(self, columns: list[str], type: str) -> Self:
        """Generate mock data

        Parameters:
            columns (list[str]): Columns to include in the mocked data
    ...
    def export(self, directory: str = None, how: str = None) -> Self:
        """Export mock data

        Mock data can be exported to a physical location by specifying the 'directory'
        argument. Otherwise specify 'how'. One of 'directory' or 'how' must be specified
        but not both.
        ...
        assert (directory or how) and not (
            directory and how
        ), "One of `dir` or `how` must be specified."
        if directory:
