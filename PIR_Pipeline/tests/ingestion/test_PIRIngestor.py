...
class TestPIRIngestor:
    ...
    def test_missing_question_error(
        self,
        dummy_ingestor,
        mock_question_data,
        mock_response_data,
        mock_missing_questions,
    ):

        mock_missing_questions = set(mock_response_data["response_merge_fail"]) - set(
            mock_question_data["question_merge_fail"]
        )

        with pytest.raises(AssertionError):
            dummy_ingestor.missing_question_error(
                mock_response_data["response_merge_fail"],
                mock_question_data["question_merge_fail"],
                mock_missing_questions,
            )

        mock_question_data = pd.DataFrame.from_dict(
            mock_question_data["question_merge_pass"]
        )
        mock_response_data = pd.DataFrame.from_dict(
            mock_response_data["response_merge_pass"]
        )
        mock_missing_questions = set(mock_response_data["question_number"]) - set(
            mock_question_data["question_number"]
        )

        expected_numrows = mock_question_data.shape[0] + len(mock_missing_questions)

        value = dummy_ingestor.missing_question_error(
            mock_response_data, mock_question_data, mock_missing_questions
        )

        assert (
            value.shape[0] == expected_numrows
        ), f"Output of mock_missing_question_error is an incorrect length. Expected {expected_numrows}."
        assert isinstance(
            value, pd.DataFrame
        ), "Output of mock_missing_question_error is not type pd.DataFrame."
        assert mock_missing_questions.issubset(
            set(value["question_number"])
        ), "Missing question not added to value."
    ...
    def test_load_data_with_valid_data(self, data_ingestor):
        data_ingestor.extract_sheets().load_data()
        assert not data_ingestor._data['question'][['question_number', 'question_name']].duplicated().any(), "Unexpected duplicate values in _data['question'][['question_name', 'question_number']]"
        
        expected_keys = ['section_a', 'section_b', 'section_c', 'section_d', 'program', 'question']
        assert ['section_a', 'section_b', 'section_c', 'section_d', 'program', 'question'] == list(data_ingestor._data.keys()), f"Output dict_keys of load_data doesn't match expected keys {expected_keys}"
        ...
        for df_name, df in data_ingestor._data.items():
            ...
            for col in df.columns:
                ...
                assert col == snake_col, f"Column '{col}' in DataFrame '{df_name}' is not in snake_case."
    ...
    def test_gen_uqid(self, dummy_ingestor):
        ...
        assert value == valid_row['uqid'], "Output uqid value doesn't match input uqid value."
        ...
        value = dummy_ingestor.gen_uqid(valid_row)
        assert value == '0cc175b9c0f1b6a831c399e269772661', "Incorrect hash returned by gen_uqid on linked_id."
...
if __name__ == "__main__":
    pytest.main([__file__, "-sk", "insert_data"])
